...
const RoleMaster = () => {
  ...
  const [newRole, setNewRole] = useState({ role: "", description: "", addGroup: "" });
  const [data, setData] = useState([
    ...
    { srNo: "5.", role: "QA", description: "Quality Assurance", addGroup: "Group A", status: "Active" },
    { srNo: "6.", role: "System Admin", description: "System Administration", addGroup: "Group C", status: "Inactive" },
    { srNo: "7.", role: "HR", description: "Human Resources", addGroup: "Group B", status: "Active" },
    { srNo: "8.", role: "Finance", description: "Financial Department", addGroup: "Group A", status: "Active" },
    { srNo: "9.", role: "Marketing", description: "Marketing Department", addGroup: "Group B", status: "Inactive" },
    { srNo: "10.", role: "Operations", description: "Operations Management", addGroup: "Group C", status: "Active" },
  ]);
  ...
  const handleAddRole = () => {
    if (!newRole.role.trim() || !newRole.description.trim() || !newRole.addGroup.trim()) {
      alert("Please fill all fields before adding a new role.");
      return;
    }
    const newEntry = {
      srNo: `${data.length + 1}.`,
      ...
      addGroup: newRole.addGroup,
      status: "Active",
    };
    setData([...data, newEntry]);
    setNewRole({ role: "", description: "", addGroup: "" });
    togglePopup();
  };
    ...
    <div className={`content-with-fixed-header px-4 flex flex-col gap-10 ${sidebarOpen ? "ml-64" : ""}`}>
      <div className="grid grid-cols-1 md:grid-cols-2 border-b pb-5">
        ...
        <div className="flex flex-col md:flex-row justify-center md:justify-end items-center gap-5">
          ...
          <div className="bg-[#d3eafd] text-[#2196f3] w-full md:w-auto h-[40px] flex justify-center items-center cursor-pointer" onClick={togglePopup}>
          ...
          <div className="bg-white p-6 rounded-lg shadow-lg w-1/3">
            ...
            <div className="flex flex-col gap-4">
              ...
              <input
                ...
                value={newRole.addGroup}
                onChange={(e) => setNewRole({ ...newRole, addGroup: e.target.value })}
              />
            </div>
            <div className="flex justify-end gap-4 mt-6">
              <button className="bg-gray-300 text-black py-2 px-4 rounded-md" onClick={togglePopup}>Cancel</button>
              <button className="bg-blue-500 text-white py-2 px-4 rounded-md" onClick={handleAddRole}>Add</button>
            </div>
          </div>
        </div>
      )}
