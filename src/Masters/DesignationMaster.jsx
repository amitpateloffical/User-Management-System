...
//             {/* Add ImportExportButtons component here */}
//             <ImportExportButtons data={data} setData={setData} fileName="Designation_Master" />
...
const DesignationMaster = () => {
  ...
  const [statusFilter, setStatusFilter] = useState("All");
  const [searchText, setSearchText] = useState("");
//   const sheetNumbers = 1; // Specify the default sheet number in code
  const [data, setData] = useState([]);
  ...
  return (
    <div className={`content-with-fixed-header px-4 flex flex-col gap-10 ${sidebarOpen ? "ml-64" : ""}`}>
      ...
      <ImportExportButtons
        ...
        sheetNumbers={1} // Pass sheet number directly here
      />
      ...
      <PopUp
        ...
        buttonText="Submit"
        labelHeading="Designation"
        inputs={[{ label: "Designation", placeholder: "Enter Designation Name" }]}
        open={popupOpen}
        onClose={togglePopup}
      />
    </div>
